...
function decodeGps(aNameIdx, aData) {
  var ret_obj = [];
  ...
  if (aData.length == 11) {
    var fixed = aData[0];
    var raw_lat = (aData[1] << 24) | (aData[2] << 16) | (aData[3] << 8) | aData[4];
...
function MatchXDecode(aData) {
  ...
  var sensorDataList = [
    ...
    { type: 22, name: "accel", idx: 0, decodeFunc: decodeGenericValue },
    { type: 23, name: "distance", idx: 0, decodeFunc: decodeGenericValue },
    { type: 24, name: "iaq", idx: 0, decodeFunc: decodeGenericValue },
          ...
          if (data_type == 1) {
            ...
            for (var i = 0; i < sensorDataList.length; i++) {
              if (sensorDataList[i].type == sensor_type) {
                if (sensor_type == 0x01) {
                  // Decode GPS data
                  var gps_data = decodeGps(sensorDataList[i].idx, aData.slice(idx + 1, idx + data_len))
                  for (var j = 0; j < gps_data.length; j++) {
                    ret_obj.push(gps_data[j]);
                  }
                }
                else {
